...
removefile(){
        rm -rf $path/package
        rm -f "$path/package.tar.gz*"
}

main(){
	ps -eaf | grep "pic.veryzhun.com/ADSB/update/package.tar.gz" | grep -v grep
	if [ $? -eq 1 ]
	then
		/usr/bin/wget -P $path -t 1 -T 2 pic.veryzhun.com/ADSB/update/package.tar.gz
        	if [ -f "$path/package.tar.gz" ]
        	then
                	dmd5=`md5sum $path/package.tar.gz|cut -d ' ' -f1`
                	if [ "$SourceMD5" = "$dmd5" ]
                	then
                        	/bin/tar -xzf $path/package.tar.gz -C $path
...
if curl -m 2 -s pic.veryzhun.com/ADSB/update.php >/dev/null;then
        FromServer=`curl -m 2 -s -d UUID=$UUID -d IpAddr=$IpAddr Device=$device  pic.veruzhun.com/ADSB/update.php`
